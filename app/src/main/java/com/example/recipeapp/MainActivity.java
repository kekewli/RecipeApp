...
public class MainActivity extends AppCompatActivity {
    ...
    @Override //Инициализация главного экрана
    protected void onCreate(Bundle savedInstanceState) {
        ...
        searchEditText.setOnEditorActionListener((v, actionId, event) -> { //Поисковик рецептов
        ...
        addRecipeButton.setOnClickListener(view -> { //Кнопка добавления рецептов
            Intent intent = new Intent(MainActivity.this, AddEditRecipeActivity.class);
            startActivity(intent);
        });
        listView.setOnItemClickListener((parent, view, position, id) -> { //Открытие рецепта из списка
            Intent intent = new Intent(MainActivity.this, AddEditRecipeActivity.class);
            intent.putExtra("RECIPE_ID", recipeIds.get(position));
            startActivity(intent);
        });
        TextView appTitleTextView = findViewById(R.id.appTitleTextView); //Пасхальная функция
